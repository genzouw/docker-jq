...
- [ ] LLM プロバイダの API キー / 従量課金 API キーを GitHub Secrets に追加していない。`GEMINI_API_KEY` / `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` 等を `secrets.*` から参照する記述が、新規ファイルだけでなく既存ファイルへの追加・変更差分にも含まれていない (PR の diff 全体を確認する)。
